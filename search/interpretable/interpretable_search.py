...
(interpretable_scaled_features_np, interpretable_feature_lst, gender_feat_np, supersense_feat_np, genre_feat_np, panel_ratio_feat_np, ) = utils.load_all_interpretable_features()
...
def get_top_n_matching_book_info(idx_top_n_np, sim_score_top_n_np, comic_info_dict=book_metadata_dict, print_n=20, query_book_id=1, feature_similarity_type='interpretable'):
...
def comics_coarse_search(query_comic_book_id: int, feature_weight_dict: dict, top_n: int):
    ...
    gender_cosine_similarity = utils.cosine_similarity(gender_feat_np[:, :], gender_feat_np[max(query_book_id, 0):query_book_id+1,  : ])
    supersense_cosine_similarity = utils.cosine_similarity(supersense_feat_np[:, :], supersense_feat_np[max(query_book_id, 0):query_book_id+1,  : ])
    genre_cosine_similarity = utils.cosine_similarity(genre_feat_np[:, :], genre_feat_np[max(query_book_id, 0):query_book_id+1,  : ])
    panel_ratio_cosine_similarity = utils.cosine_similarity(panel_ratio_feat_np[:, :], panel_ratio_feat_np[max(query_book_id, 0):query_book_id+1,  : ])
    ...
    combined_results_similarity = gender_cosine_similarity *  feature_weight_dict['gender'] + supersense_cosine_similarity *  feature_weight_dict['super_sense'] + genre_cosine_similarity *  feature_weight_dict['genre'] + panel_ratio_cosine_similarity *  feature_weight_dict['story_pace'] 
    ...
    top_k_df = get_top_n_matching_book_info(idx_top_n_np=combined_results_indices, sim_score_top_n_np=combined_sorted_result_indices, comic_info_dict=book_metadata_dict, print_n=top_n, query_book_id=query_book_id, feature_similarity_type= 'interpretable_combined')
...
if __name__ == "__main__":
    ...
    top_n_results_df = comics_coarse_search(query_comic_book_id=anchors_book_idx[0], feature_weight_dict=feature_importance,top_n=top_n)
